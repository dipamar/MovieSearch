...
$(document).ready(() =>{
	...
	//if (title & year) search 
	$('#searchByTitleAndYear').click(() =>{
		search = 2;
		validationData();
	});
	//if (id) search 
	$('#searchByIMDBId').click(() =>{
		search = 3;
		validationData();
	});
})
...
let validationData = () =>{
	...
	if(search == 1){
		...
		//assigning link and calling getMovieDetails()
			...
			link = `https://www.omdbapi.com/?apikey=96023284&t=${title}`;
			getMovieDetails();
		}
	}
	//for search by title & year
		...
		//assigning link and calling getMovieDetails()
			...
			//to add '+' in place of space
			title = title.split(' ').join('+');
			link = `https://www.omdbapi.com/?apikey=96023284&t=${title}&y=${year}`;
			getMovieDetails();
		}
	}
	//for id
		...
		//assigning link and calling getMovieDetails()
			...
			link = `https://www.omdbapi.com/?apikey=96023284&i=${id}`;
			getMovieDetails();
		}
	}
}
		...
		success: (response) =>{
			...
			if(response.Response == "True"){
				...
				if(response.Poster == "N/A"){
					$("#poster").html(`<img src="substitute.jpg" style="margin-left: 2rem;margin-top: -4rem;height:50vh;width:18vw;border: 1px solid white;padding: 0.5rem; background-color: white;">`);
					$("#pCon").html(`<img src="substitute.jpg" style="margin-left: 2rem;height:50vh;border: 1px solid white;padding: 0.5rem; background-color: white;">`);
				}
				else{
